...
class inferenceGPT:
    ...
    def metrics_function_all_details(self, l_pred, l_real, l_pred_all_24_features,l_real_all_24_features  ):
        ...
        ##########################
        ##a_list = np.reshape( l_real_all_24_features[-self.how_many:, 2], (-1) )
        ##b_list = np.reshape( l_pred_all_24_features[-self.how_many:, 2], (-1) )
        ##for jj in range(a_list.shape[0]:
        ##    r2_SI_f2_t1_t1_t3_tn   = r2_score(. a_list[jj], b_list[jj]  ) 
        ##    print("R2 for SI (f2) at t", str(jj) , "....."
        ##########################
